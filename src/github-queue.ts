...
function validateDelivery(payload: DraftPullRequestDelivery): void {
  if (!Number.isInteger(payload.issueNumber) || payload.issueNumber <= 0 || payload.status !== "ready_for_main" || (payload.reviewerVerdict !== "pass" && !(payload.reviewerVerdict === "escalate" && payload.tier === 2)) || !["pass", "not_run"].includes(payload.finalReviewerVerdict) || (payload.tier === 2 && payload.finalReviewerVerdict !== "pass")) throw new Error("incomplete delivery evidence; no PR was published");
  ...
  if (!payload.git.files.length || payload.git.files.length > MAX_DELIVERY_LIST_ITEMS || !Number.isInteger(payload.git.filesChanged) || payload.git.filesChanged !== payload.git.files.length || !Number.isInteger(payload.git.insertions) || payload.git.insertions < 0 || !Number.isInteger(payload.git.deletions) || payload.git.deletions < 0) throw new Error("missing Git delivery evidence; no PR was published");
